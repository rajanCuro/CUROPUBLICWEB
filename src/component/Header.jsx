...
const Header = () => {
  ...
  const { screen, getAllLabCartItems, labCartItems } = useLabAuth();
  ...
  // Update placeholder text every 3 seconds
  useEffect(() => {
  ...
  return (
    <header className="bg-white shadow-sm border-b border-gray-100 sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center flex-shrink-0">
            <Link to="/" className="flex items-center space-x-3 group">
              <img src={logo} alt="Company Logo" className="md:h-5 h-5 w-auto transition-transform duration-300 group-hover:scale-105" />
          ...
          <div className="hidden lg:flex items-center space-x-1 mx-8 flex-1 justify-center">
            {navigationItems.map((item, index) => (
              <motion.div key={item.name} initial="hidden" animate="visible" variants={itemVariants} custom={index}>
                <Link
                  to={item.path}
                ...
                >
                  <span className={`transition-colors duration-300 ${activeMenu === item.name ? 'text-teal-600' : 'text-gray-500 group-hover:text-teal-600'}`}>
                    {item.icon}
                  </span>
                  <span className="text-sm font-semibold">{item.name}</span>
                  <div className={`absolute inset-0 rounded-lg bg-gradient-to-r from-teal-500/10 to-emerald-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${activeMenu === item.name ? 'opacity-100' : ''}`}></div>
                </Link>
              </motion.div>
            ))}
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            {token ? (
              <motion.div ref={profileRef} className="relative">
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setProfileOpen(!profileOpen)} className="flex items-center space-x-2 p-2 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <IoIosArrowDown className={`text-gray-400 transition-transform duration-200 ${profileOpen ? 'rotate-180' : ''}`} size={14} />
                </motion.button>

                {/* Profile Dropdown */}
                <AnimatePresence>
                  {profileOpen && (
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-50">
                      ...
                      <button onClick={() => { setProfileOpen(false); navigate("/manage_profile"); }} className="w-full flex items-center space-x-3 px-4 py-3 text-left text-gray-700 hover:bg-teal-50 hover:text-teal-600 transition-colors duration-200">
                        <MdOutlineManageAccounts size={18} />
                        <span className="text-sm">Manage Profile</span>
                      </button>

                      <button onClick={() => { setProfileOpen(false); navigate("/medicine/order"); }} className="w-full flex items-center space-x-3 px-4 py-3 text-left text-gray-700 hover:bg-teal-50 hover:text-teal-600 transition-colors duration-200">
                        <FaHistory size={16} />
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setAuthModal(true)} className="hidden sm:flex items-center space-x-2 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white px-4 py-2 rounded-lg transition-all duration-200 shadow-sm">
      ...
      <AnimatePresence>
        {searchOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSearchOpen(false)} className="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 md:hidden" />
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="fixed top-4 left-4 right-4 bg-white rounded-xl shadow-xl z-50 md:hidden p-4">
              <div className="flex items-center space-x-3">
                <div className="flex-1 flex items-center bg-gray-100 border border-gray-200 px-4 py-3 rounded-lg">
                  ...
                  <input type="text" placeholder={placeholders[placeholderIndex]} className="bg-transparent outline-none text-sm w-full border-0" autoFocus />
                </div>
                <button onClick={() => setSearchOpen(false)} className="p-2 text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-100">
                  <FiX size={20} />
                </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: '100%' }} transition={{ type: "spring", damping: 30 }} className="fixed inset-0 bg-white z-40 lg:hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <img src={logo} alt="Logo" className="h-8" />
              <button onClick={() => setMenuOpen(false)} className="p-2 text-gray-500 hover:text-gray-700">
                <FiX size={24} />
              </button>
            </div>
            ...
            <div className="p-4 space-y-2">
              {navigationItems.map((item) => (
                <Link key={item.name} to={item.path} onClick={() => setMenuOpen(false)} className={`flex items-center space-x-3 p-3 rounded-lg transition-colors ${activeMenu === item.name ? "bg-teal-50 text-teal-600 font-semibold" : "text-gray-700 hover:bg-gray-50"}`}>
                  {item.icon}
                  <span>{item.name}</span>
                </Link>
              ))}
              ...
              {!token && (
                <button onClick={() => { setMenuOpen(false); setAuthModal(true); }} className="w-full flex items-center justify-center space-x-2 bg-gradient-to-r from-teal-600 to-emerald-600 text-white py-3 rounded-lg font-semibold mt-4">
      ...
      <div className="card flex justify-content-center">
        <Sidebar visible={visible} onHide={() => setVisible(false)} position="right" style={{ width: "50vw" }}>
          <Map />
        </Sidebar>
      </div>
    </header>
